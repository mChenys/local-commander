...
# 添加当前目录到 Python 路径，确保能找到同目录下的模块
...
class MCPRouterServer:
            ...
            # ========== VL 视觉定位工具（多级降级：remote_vl -> local_vl -> dump_ui）==========
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        "image_path": {
                            "type": "string",
                            "description": "图像文件路径（支持 Android/Web 截图）"
                        },
                        "prompt": {
                            "type": "string",
    ...
    async def _call_tool(self, request_id: Any, tool_name: str, args: dict) -> dict:
        ...
        try:
            ...
            # ========== VL 视觉定位工具实现 ==========
            elif tool_name == "vl_detect_element":
                result = self._vl_detect_element(args)
                return self._response(request_id, {
                    "content": [{
                        "type": "text",
                        "text": json.dumps(result, ensure_ascii=False, indent=2)
            ...
            elif tool_name == "vl_get_click_coords":
                result = self._vl_get_click_coords(args)
                return self._response(request_id, {
                    "content": [{
                        "type": "text",
                        "text": json.dumps(result, ensure_ascii=False, indent=2)
...
'''
    ...
    def _android_dump_ui(self, args: dict) -> dict:
        ...
        try:
            ...
            # 统计可点击元素
            clickable_count = sum(1 for e in all_elements if e.get("clickable"))

            return {
                "success": True,
                "device": ui.get("device"),
                "total_elements": ui.get("element_count", 0),
                "visible_elements": len(all_elements),
    ...
    # ========== VL 视觉定位工具实现 ==========
    def _get_vl_grounding_service(self):
        """获取 VL Grounding 服务实例"""
        try:
            from vl_grounding import get_vl_grounding_service
            return get_vl_grounding_service()
